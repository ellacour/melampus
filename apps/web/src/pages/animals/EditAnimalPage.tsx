/**
 * Modification d'un dossier animal — `/animals/:id/edit`.
 *
 * Charge l'animal via GET /animals/:id/, pré-remplit le formulaire, envoie
 * un PATCH partiel à la soumission. Structure en sections identique à la
 * création ; la section "Profil sanitaire" s'ouvre automatiquement si des
 * champs avancés sont déjà renseignés.
 */
...
// Edit form includes 'unknown' — an existing animal may already have it.
...
const editAnimalSchema = z
  .object({
    name: z.string().trim().min(1, 'Le nom est requis.').max(80, '80 caractères maximum.'),
    species: z.enum(['canine', 'feline', 'equine', 'bovine', 'ovine', 'caprine', 'porcine', 'avian', 'other']),
    gender: z.enum(['male', 'female', 'unknown']),
    main_usage: z.enum(['leisure', 'boarding', 'competition', 'breeding', 'racing', 'sales', 'export', 'company', 'other', 'unknown']),
    ...
    country: z.string().length(2, 'Code pays sur 2 lettres (ex : FR).'),
    department_code: z
      .string()
      .trim()
    ...
    living_context: z
      ...
      .optional(),
    travels_outside_home: z.boolean(),
    external_animals_contact: z.boolean(),
    has_young_or_pregnant_animals_on_site: z.boolean(),
    is_breeding_animal: z.boolean(),
    reproductive_status: z.enum([
  ...
  .superRefine((data, ctx) => {
    if (data.gender === 'male' && (data.reproductive_status === 'pregnant' || data.reproductive_status === 'with_young')) {
      ctx.addIssue({
...
function animalToFormValues(animal: Animal): EditAnimalForm {
  return {
    name: animal.name,
    species: animal.species,
    gender: animal.gender,
    main_usage: animal.main_usage,
    breed: animal.breed ?? '',
    birth_date: animal.birth_date ?? '',
    identification_number: animal.identification_number ?? '',
    country: animal.country ?? 'FR',
    department_code: animal.department_code ?? '',
    living_context: animal.living_context !== 'unknown' ? animal.living_context : undefined,
    travels_outside_home: animal.travels_outside_home ?? false,
    external_animals_contact: animal.external_animals_contact ?? false,
    has_young_or_pregnant_animals_on_site: animal.has_young_or_pregnant_animals_on_site ?? false,
    is_breeding_animal: animal.is_breeding_animal ?? false,
    reproductive_status: animal.reproductive_status ?? 'not_applicable',
...
function hasNonDefaultHealthProfile(animal: Animal): boolean {
  return (
    (animal.living_context !== 'unknown' && !!animal.living_context) ||
    animal.travels_outside_home ||
    animal.external_animals_contact ||
    animal.has_young_or_pregnant_animals_on_site ||
    animal.is_breeding_animal ||
    animal.reproductive_status !== 'not_applicable'
  )
}
...
export function EditAnimalPage() {
  ...
  useEffect(() => {
    if (animal) {
      reset(animalToFormValues(animal))
      setHealthProfileOpen(hasNonDefaultHealthProfile(animal))
    }
  }, [animal, reset])
  ...
  const mutation = useMutation({
    mutationFn: async (data: EditAnimalForm) => {
      const payload = {
        name: data.name,
      ...
      const res = await apiClient.patch<Animal>(`/animals/${id}/`, payload)
      return res.data
    },
    onSuccess: (updated) => {
      queryClient.setQueryData(['animals', id], updated)
      void queryClient.invalidateQueries({ queryKey: ['animals'] })
      navigate(`/animals/${id}`)
    },
    onError: () => {
      setError('root', {
        message: 'Impossible de sauvegarder les modifications. Vérifiez les champs ou réessayez.',
      })
    },
  })

  if (isLoading) return <LoadingState />
  if (error || !animal) return <ErrorState id={id} />
  ...
  return (
    <div className="max-w-[760px]">
      <header className="mb-10">
        <Link
          to={`/animals/${id}`}
          className="inline-flex items-center gap-2 mb-5 font-mono text-[11px] font-medium uppercase tracking-[0.08em] text-ink-soft hover:text-ink no-underline transition-colors"
        >
      ...
      >
        {/* 01 — Identité */}
        <FormSection num="01" title="Identité" subtitle="Nom, espèce, sexe et usage principal.">
          ...
          <FieldRow>
            <Field label="Espèce" error={errors.species?.message} required>
              <select className={`input ${errors.species ? 'is-error' : ''}`} {...register('species')}>
            ...
            <Field label="Sexe" error={errors.gender?.message} required>
              <select className={`input ${errors.gender ? 'is-error' : ''}`} {...register('gender')}>
          ...
          <Field label="Usage principal" help="Détermine les règles sanitaires et vaccinales applicables." error={errors.main_usage?.message} required>
            <select className={`input ${errors.main_usage ? 'is-error' : ''}`} {...register('main_usage')}>
          ...
          <Field label="Race" help="Optionnel · Border collie, Européen, Pur-sang…" error={errors.breed?.message}>
            <input type="text" className={`input ${errors.breed ? 'is-error' : ''}`} autoComplete="off" {...register('breed')} />
          </Field>
        </FormSection>

        {/* 02 — Naissance & identification */}
        <FormSection num="02" title="Naissance et identification" subtitle="Pour calculer l'âge et associer les protocoles vétérinaires applicables.">
          <FieldRow>
            <Field label="Date de naissance" help="Optionnel — si inconnue, laissez vide." error={errors.birth_date?.message}>
              <input type="date" className={`input ${errors.birth_date ? 'is-error' : ''}`} {...register('birth_date')} />
            </Field>
            <Field label="Numéro d'identification" help="Puce, ICAD, SIRE…" error={errors.identification_number?.message}>
              <input type="text" className={`input ${errors.identification_number ? 'is-error' : ''}`} autoComplete="off" {...register('identification_number')} />
            </Field>
          </FieldRow>
        </FormSection>

        {/* 03 — Localisation */}
        <FormSection num="03" title="Localisation" subtitle="Le département conditionne certaines obligations vaccinales.">
          <FieldRow>
            <Field label="Pays" help="Code ISO 2 lettres." error={errors.country?.message} required>
              <input
                type="text"
                maxLength={2}
                className={`input max-w-[100px] uppercase ${errors.country ? 'is-error' : ''}`}
                autoComplete="off"
                {...register('country')}
              />
            </Field>
            <Field label="Département" help="Optionnel — code FR, 2 chiffres ou 2A/2B." error={errors.department_code?.message}>
              <input
                type="text"
                maxLength={3}
        ...
        <section className="border border-line rounded-sm">
          ...
          >
            <div>
              ...
              <p className="text-[13px] text-ink-soft leading-snug max-w-[200px]">
                Contexte de vie, expositions, reproduction.
              </p>
            </div>
            <div className="flex items-center gap-3 text-[13px] text-ink-soft">
              <span>{healthProfileOpen ? 'Réduire' : 'Modifier'}</span>
              <svg
          ...
          {healthProfileOpen && (
            <div className="px-6 pb-6 space-y-6 border-t border-line pt-6">
              <Field label="Contexte de vie" help="Comment l'animal est-il hébergé au quotidien ?" error={errors.living_context?.message}>
                <select className={`input ${errors.living_context ? 'is-error' : ''}`} {...register('living_context')}>
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-[14px] text-ink">
                    ...
                    <span className="block text-[12.5px] text-ink-soft mt-0.5">Concours, transport, stages, saillie, soins à l'extérieur…</span>
                  </span>
                </label>
                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" className="mt-0.5" {...register('external_animals_contact')} />
                  <span className="text-[14px] text-ink">En contact avec des animaux extérieurs au groupe habituel</span>
                </label>
                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" className="mt-0.5" {...register('has_young_or_pregnant_animals_on_site')} />
                  <span className="text-[14px] text-ink">Jeunes animaux ou femelles gestantes présents sur le site</span>
                </label>
              </div>
              ...
              <div className="space-y-4">
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  <span className="text-[14px] text-ink">Animal utilisé ou destiné à la reproduction</span>
                </label>

                {showReproductiveFields && (
                  <Field label="Statut reproducteur" error={errors.reproductive_status?.message}>
                    <select className={`input ${errors.reproductive_status ? 'is-error' : ''}`} {...register('reproductive_status')}>
                ...
                {showExpectedBirthDate && (
                  <Field label="Date prévue de mise bas" help="Requise si l'animal est gestant." error={errors.expected_birth_date?.message} required>
                    <input type="date" className={`input ${errors.expected_birth_date ? 'is-error' : ''}`} {...register('expected_birth_date')} />
                  </Field>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-line">
                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" className="mt-0.5" {...register('is_active')} />
                  <span className="text-[14px] text-ink">
                    Dossier actif
                    <span className="block text-[12.5px] text-ink-soft mt-0.5">Décochez si l'animal est vendu, décédé ou archivé. Les rappels seront suspendus.</span>
                  </span>
                </label>
              </div>
            </div>
        ...
        {/* 05 — Notes */}
        <FormSection num="05" title="Notes" subtitle="Visible uniquement par vous.">
        ...
        <div className="flex justify-end items-center gap-3 pt-6 border-t border-line">
          <Link to={`/animals/${id}`} className="btn-secondary">
            Annuler
          </Link>
          <button
            type="submit"
...
function LoadingState() {
  return (
    <div className="space-y-8 animate-pulse max-w-[760px]">
      <div className="h-3 w-32 bg-line rounded-sm" />
      <div className="h-10 w-64 bg-line rounded-sm" />
      <div className="h-[220px] bg-white rounded-md border border-line" />
      <div className="h-[160px] bg-white rounded-md border border-line" />
      <div className="h-[100px] bg-white rounded-md border border-line" />
    </div>
  )
}

function ErrorState({ id }: { id: string | undefined }) {
  return (
    <div className="max-w-[640px]">
      <div className="rounded-md border border-status-over/40 bg-status-over-tint/40 p-6">
        ...
        <p className="text-ink mb-4">
          Impossible de charger ce dossier pour le modifier.
        </p>
        <Link to={id ? `/animals/${id}` : '/animals'} className="btn-secondary inline-flex">
          Retour au dossier
        </Link>
      </div>
    </div>
...
function FormSection({ num, title, subtitle, children }: { num: string; title: string; subtitle?: string; children: ReactNode }) {
  return (
    <section className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-x-10 gap-y-4 pb-2">
      <div>
        ...
        {subtitle && (
          <p className="text-[13px] text-ink-soft leading-snug max-w-[200px]">{subtitle}</p>
        )}
      </div>
  ...
  return (
    <div>
      <label className="label flex items-center justify-between mb-1.5">
        <span>{label}</span>
        {required && (
          <span className="font-mono text-[9px] tracking-[0.06em] text-ink-soft normal-case">Requis</span>
        )}
      </label>
      {children}
      {error ? (
